...
import { getAdmissionsVisibleStudentProfileReadOnly } from '@/lib/admissions-discovery/queries';
import { saveToShortlistAction } from './actions';
import { SaveToShortlistForm } from './save-shortlist-form';
...
export default async function AdmissionsStudentProfilePage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  await requireRouteAccess('/admissions/dashboard');

  const { id } = await params;

  const aggregate = await getAdmissionsVisibleStudentProfileReadOnly(id);

  const backLink = (
    <p className="mt-32">
      <Link href="/admissions/dashboard" className={pillLinkClasses}>
  ...
  return (
    <main className="content-wrapper py-40">
      ...
      <p className="text-body text-slate-gray mb-32">
        Read-only view of a visible student profile.
      </p>

      <div className="flex flex-col gap-24">

        <Card>
          <SaveToShortlistForm action={saveToShortlistAction} studentProfileId={id} />
        </Card>
